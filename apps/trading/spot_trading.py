...
from apps.utils.binance_spot import safe_binance_call, get_spot_balance, get_current_capital, get_lot_size_info, buy_spot_position, sell_spot_position
...
def run_trading_bot(symbol, timeframe, risk_per_trade, rsi_window, rsi_entry, rsi_exit, rsi_exit_exec):
    try:
        while True:
            # Define global variables   
            step_size, min_qty     = get_lot_size_info(symbol)
            log_dir                = Path("logs/trades/spot")
            ...
            while True:
                ...
                if current_time - last_memory_check >= 300: # Log memory usage every 5 minutes
                ...
                buy_orders_file        = log_path('buy_orders', '.jsonl')
                sell_orders_file       = log_path('sell_orders', '.jsonl')
                ...
                if not currently_trading:
                    if rsi < rsi_entry:   
                        ...
                        # TRADE
                        order_data = buy_spot_position(symbol, risk_per_trade, step_size, min_qty)

                        # Save original JSON log file
                        Path(buy_orders_file).parent.mkdir(parents=True, exist_ok=True)

                        with open(buy_orders_file, "a") as f:
                            f.write(json.dumps(order_data) + "\n")
                        
                        async_sharepoint_file("upload", buy_orders_file)

                        # Extract trade data
                        symbol                   = order_data["symbol"]
                        entry_time               = datetime.utcfromtimestamp(order_data["transactTime"] / 1000).strftime("%Y-%m-%d %H:%M:%S")
                        
                        fills                    = order_data["fills"]
                        entry_price              = round(sum(float(f["price"]) for f in fills) / len(fills), 2)
                        order_type               = order_data["type"]
                        trading_amount           = round(float(order_data["cummulativeQuoteQty"]), 2)

                        # Commission costs
                        commission               = sum(float(f["commission"]) for f in fills)
                        commission_asset         = fills[0]["commissionAsset"]
                        entry_commission         = commission if commission_asset == "USDT" else commission * entry_price
                    ...
                    if rsi_was_above_exit and rsi < rsi_exit_exec: 
                        ...
                        # TRADE
                        order_data = sell_spot_position(symbol, step_size, min_qty)

                        # Save original JSON log file
                        Path(buy_orders_file).parent.mkdir(parents=True, exist_ok=True)

                        with open(sell_orders_file, "a") as f:
                            f.write(json.dumps(order_data) + "\n")
                        
                        async_sharepoint_file("upload", sell_orders_file)

                        # Extract trade data
                        symbol                   = order_data["symbol"]
                        exit_time                = datetime.utcfromtimestamp(float(order_data["transactTime"]) / 1000).strftime("%Y-%m-%d %H:%M:%S")
                        
                        fills                    = order_data["fills"]
                        exit_price               = round(sum(float(f["price"]) for f in fills) / len(fills), 2)
                        order_type               = order_data["type"]
                        trading_amount           = round(float(order_data["cummulativeQuoteQty"]), 2)
                        ...
                        commission               = sum(float(f["commission"]) for f in fills)
                        commission_asset         = fills[0]["commissionAsset"]
                        exit_commission          = commission if commission_asset == "USDT" else commission * float(entry_price)
                        
                        # PnL calculations
                        current_capital          = get_current_capital()
                        
                        df_trade = pd.read_csv(active_trade_file)
                        capital_before           = df_trade["Capital Before"].iloc[-1]
                        entry_time               = df_trade["Entry Time"].iloc[-1]
                        del df_trade
                        gc.collect()
                        
                        realized_pnl             = current_capital - capital_before
                        realized_pnl_pct         = realized_pnl / capital_before if capital_before != 0 else 0
...
if __name__ == "__main__":
    ...
    if mode == "check":
        ...
        try:
            ...
            error_output = {"Error in 'spot_trading.py'. Error": f"{type(e).__name__}: {str(e)}"}
